...
if ($act=='login'){
    ...
    if($row){
        ...
        print_msg("Salah kombinasi username dan/atau password.");
    }
}
...
if($mod=='alternatif_tambah'){
    $kode = $_POST['kode'];
    $nama = $_POST['nama'];
    $wilayah =$_POST['getwilayah'];

    if($kode=='' || $nama=='' || $wilayah=='')
        print_msg("Field bertanda * tidak boleh kosong!");

   elseif($db->get_results("SELECT * FROM tb_alternatif WHERE kode_alternatif='$kode'"))
        print_msg("Kode sudah ada!");
    else{
        $db->query("INSERT INTO tb_alternatif (kode_alternatif, nama_alternatif,id_kategori_wilayah)
            VALUES ('$kode', '$nama','$wilayah')");
    ...
    if($kode=='' || $nama=='' || $wilayah=='')
        ...
        $db->query("UPDATE tb_alternatif SET nama_alternatif='$nama',id_kategori_wilayah='$wilayah'  WHERE kode_alternatif='$_GET[ID]'");
        redirect_js("index.php?m=alternatif");
    }
...
/** relasi */
  ...
  $totalKriteria= $db->get_var("SELECT count(*) FROM tb_kriteria");

  foreach ($_POST['id-alternatif'] as $key => $value) {
    for ($i=0; $i < $totalKriteria ; $i++) {
      if(strlen((string)$i)>0 && strlen((string)$i)<2 ){
        $idKriteria= 'C0'.($i+1);
      }else{
        $idKriteria= 'C'.($i+1);
      }
      $post=$_POST[$idKriteria];
      $db->query("UPDATE tb_rel_alternatif SET nilai='$post[$key][$i]' WHERE kode_kriteria='$idKriteria'  AND kode_alternatif ='$value' ");
    }
  }
//
