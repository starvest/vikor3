...
function get_wilayah(){
  global $wilayah;
  $rows= $wilayah->get_results("SELECT * FROM tb_kategori_wilayah");
  $wil = array();
  foreach ($rows as $row) {
    $wil[$rows->id]=$rows->wilayah;
  }
  return $wil;
}
...
function get_checklist(){
    global $checklist;
    $rows= $checklist->get_results("SELECT * FROM tb_alternatif");
    $cl = array();
    foreach ($rows as $row) {
      $cl[$rows->id]=$rows->checklist;
    }
    return $cl;
  }
...
function get_nama(){
    global $nama;
    $rows= $nama->get_results("SELECT * FROM tb_alternatif");
    $nam = array();
    foreach ($rows as $row) {
      $nam[$rows->id]=$rows->nama_alternatif;
    }
    return $nam;
  }
